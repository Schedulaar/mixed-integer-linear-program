...
\begin{proof}
	...
	\begin{description}
		...
		Für die Seitenfläche $F\subseteq P$, die alle optimalen Lösungen von ($\emptyset$-\MIPI) enthält, sei $z$ eine Ecke, die $x\mapsto d\transpose x$ über $F$ maximiert.
		Ist $x^*\in F$, so ist $x^*$ bereits optimal für ($\emptyset$-\MIPI).
		Sonst setze $\lambda\geq 0$ so groß, dass $\lambda\geq d\transpose (v-z)/c\transpose (z-v)$ für alle Ecken $v\in P\setminus F$ von $P$ gilt.
		Mit $\tilde{c}:= \lambda c + d$ maximiert $z$ die Funktion $x\mapsto \tilde{c}\transpose x$ über $P$, denn für alle Ecken $v$ in  P gilt: Ist $v\in F$, dann ist $\tilde{c}\transpose v = \lambda c\transpose z + \lambda d\transpose v\leq \tilde{c}\transpose z$, und sonst ist 
		$\tilde{c}\transpose (z-v)\geq (d\transpose (v-z)/c\transpose(z-v))
			 c\transpose (z-v) + d\transpose (z-v)=0$
